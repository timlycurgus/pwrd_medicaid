...
summarize_det_dat <- function(year, icd10codes = icd10_regexps){
  ...
  f.path <- file.path(file.name)
  
  mort_detail  <-  f.path %>%
    read_fwf(fwf_cols(rec_type=c(19,19), # 'record type'
                      staterFIPS=c(33,34), # state of residence
            ...
            rcon8=col_character(),
            rcon9=col_character(),
            rcon10=col_character(),
            rcon11=col_character(),
            rcon12=col_character(),
  ...
  mort_detail <- mort_detail %>% 
    ...
    mutate(race = case_when(race_code == "01" & hisp_code %in% non_hisp_codes & is.na(race_imp) ~ "NHWA",
                            race_code == "02" & hisp_code %in% non_hisp_codes & is.na(race_imp) ~ "NHBA",
                            race_code == "03" & hisp_code %in% non_hisp_codes & is.na(race_imp) ~ "NHIA",
                            race_code %in% asian & hisp_code %in% non_hisp_codes & is.na(race_imp) ~ "NHAA",
                            race_code %in% pac_island & hisp_code %in% non_hisp_codes & is.na(race_imp) ~ "O",
                            hisp_code %in% hisp_codes ~ "H",
                            TRUE ~ "O"),
           race_bridged = case_when(race_rec5 == 1 & hisp_code %in% c(non_hisp_codes,9) ~ "NHWA",
                                    race_rec5 == 2 & hisp_code %in% c(non_hisp_codes,9) ~ "NHBA",
                                    race_rec5 == 3 & hisp_code %in% c(non_hisp_codes,9) ~ "NHIA",
                                    race_rec5 == 4 & hisp_code %in% c(non_hisp_codes,9) ~ "NHAA",
                                    race_rec5 == 5 & hisp_code %in% c(non_hisp_codes,9) ~ "O",
  ...
  mort_det_sum <- mort_detail %>%
    group_by(staterFIPS, cntyrFIPS, year, race, race_bridged, age,sex, c_o_d_cat) %>%
    summarize(mort = n())
  
  #save year's data
  save(mort_det_sum, file = paste0("det_mort_", year ,".Rdata"))
}
...
save(mort_det_comb, file = file.path("..","..","..", "compute1","data","raw_all_det_mort.Rdata"))

table(mort_det_comb$race, mort_det_comb$race_bridged)
...
mort_det <- mort_det %>%
  dplyr::select(staterFIPS:age, mort_HC_amenable_not_flu = mort_HC_amenable_not_flu_0, 
                mort_Flu = mort_Flu_0, mort_all_cause) %>%
  pivot_wider(names_from = year, values_from = c(mort_HC_amenable_not_flu,mort_Flu , mort_all_cause),
